...
ewas_qgcomp_fit.boot<-bplapply(X=meth_Female_list_matrix, FUN=ewas_qgcomp.boot, pheno = Pheno_Female, mix_comp = Xnm, covars = covars, mval_conversion=FALSE, qval=4, output_type="full", Bval=1000, seedval=1234, BPPARAM = SnowParam(workers = 20))
end <- Sys.time()
as_hms(difftime(end, start))

### Save EWAS QGComp Results

save(ewas_qgcomp_fit.boot,file="Female_BootstrapResults_03182024.RData")
