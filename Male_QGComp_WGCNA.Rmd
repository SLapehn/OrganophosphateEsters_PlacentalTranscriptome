...
## Male Stratified- QGComp for OPE Data
...
MEs_forOPE <- MEs %>%
  dplyr::filter(row.names(MEs) %in% row.names(OPE_Data_Select_Ready_Male))
...
Mixture <- OPE_Data_Select_Ready_Male %>%
...
```{r covar prep}
Covariates_Selected <- Covariates_Male %>%
...
```{r save BS}
#save(ewas_qgcomp_fit.boot, file="/active/paquette_a/slapehn/OPE_and_PlacentalTranscriptome/Results/EWAS_QGComp/Male_BoostrapResults_WGCNA_110923.RData")
...
comp_effects_BS_sig <- comp_effects_BS %>%
  dplyr::filter(probeID %in% Results_BS_Sig$probeID)

#Make separate comp effects dataframes for each Significant (p<0.05) module

comp_effects_BS_brown <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEbrown")
comp_effects_BS_brown$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_cyan <- comp_effects_BS %>%
  dplyr::filter(probeID =="MEcyan")
comp_effects_BS_cyan$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_darkred <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEdarkred")
comp_effects_BS_darkred$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")
...
comp_effects_BS_grey <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEgrey") #This is technically the module for all the extra genes
comp_effects_BS_grey$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_midnightblue <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEmidnightblue")
comp_effects_BS_midnightblue$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_royalblue <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEroyalblue")
comp_effects_BS_royalblue$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_white <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEwhite")
comp_effects_BS_white$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")

comp_effects_BS_yellow <- comp_effects_BS %>%
  dplyr::filter(probeID == "MEyellow")
comp_effects_BS_yellow$OPE <- c("DEP", "DBUP & DIBP", "BCETP", "BDCPP", "DPHP")
```
...
```{r plot CE}
ggplot(comp_effects_BS_brown, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-Brown (Male: N=361)")

ggplot(comp_effects_BS_cyan, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-Cyan (Male: N=361)")

ggplot(comp_effects_BS_darkred, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-DarkRed (Male: N=361)")
...
ggplot(comp_effects_BS_greenyellow, aes(x=OPE, y=weight, fill=OPE)) + 
  ...
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-GreenYellow (Male: N=361)")

ggplot(comp_effects_BS_grey, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-Grey (Male: N=361)")

ggplot(comp_effects_BS_midnightblue, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-MidnightBlue (Male: N=361)")

ggplot(comp_effects_BS_royalblue, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-RoyalBlue (Male: N=361)")

ggplot(comp_effects_BS_white, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-White (Male: N=361)")

ggplot(comp_effects_BS_yellow, aes(x=OPE, y=weight, fill=OPE)) + 
  geom_bar(stat="identity") + 
  theme_bw() + 
  scale_y_continuous(limits = c(-0.005, 0.005)) + 
  geom_hline(yintercept = 0) + 
  coord_flip() + 
  ggtitle("Component Weights-Yellow (Male: N=361)")

# Sum overall mixture effects
sum(comp_effects_BS_brown$weight)
sum(comp_effects_BS_cyan$weight)
sum(comp_effects_BS_darkred$weight)
sum(comp_effects_BS_greenyellow$weight)
sum(comp_effects_BS_grey$weight)
sum(comp_effects_BS_midnightblue$weight)
sum(comp_effects_BS_royalblue$weight)
sum(comp_effects_BS_white$weight)
sum(comp_effects_BS_yellow$weight)
