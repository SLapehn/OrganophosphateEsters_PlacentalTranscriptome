---
title: "Male_IndividualOPE_DEGAnalysis"
...
## Male-Individual OPE DEG Analysis

This code will be used to evaluate the association between individual OPE compounds and the placental transcriptome in male samples only. Covariates and Exposure Data have already been cleaned/processed and RNAseq data has been TMM normalized and filtered. Latest version of all data is from 10/23/23. 
...
```{r DEG Analysis}
OPE_Results<-list()
for(i in 1:dim(OPE_Data_Select_Ready_Male)[2]){
mod <-model.matrix(~+(OPE_Data_Select_Ready_Male[,i])
 ...
 mod0=mod[,-2]
  sv <- sva(cpm(Y_norm_Male, log = TRUE),mod, mod0)
  mod_sv<-cbind(mod,sv$sv)
  dim(mod)
  
  v<-voom(Y_norm_Male, mod_sv, plot=TRUE) 
...
par(mfrow=c(1,2))
hist(Results$P.Value,breaks=20,main=colnames(OPE_Data_Select_Ready_Male)[i])
plot(Results$logFC,-log(Results$P.Value, base=10),ylim=c(0,15),pch=16,cex=0.5,main=colnames(OPE_Data_Select_Ready_Male)[i],ylab="Log P Value",xlab="logFC")
  ...
  names(OPE_Results)[i]<-colnames(OPE_Data_Select_Ready_Male)[i]
}

#save(OPE_Results, file="/active/paquette_a/slapehn/OPE_and_PlacentalTranscriptome/Results/Gene Results/OPE_Results_MaleModel_083024.Rdata")
```
...
#write.xlsx(names_DEGs, rowNames=TRUE, file = '/active/paquette_a/slapehn/OPE_and_PlacentalTranscriptome/Results/Gene Results/MaleModelDEGs_083024.xlsx')
```
